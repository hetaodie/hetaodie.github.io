...
# **回答:**
# 索引65, 66, 75, 128, 154的数据被认为是在每个特征情况下是异常数据的情况超过一次, 因此需要移除, 否则计算误差会变大
...
# **回答:**
#  - 总共表示了0.7085的方差 - 前四个表示了0.9311的方差 - 表示零售商和咖啡店，从第一个维度来看，可以分成两部分，第一部分Milk、Grocery和DP他们是负权特征，彼此之间是相关性最强，第二个维度上，Fresh、Frozen和Delicatessen彼此之间相关性最强，第三个维度体现了Frozen和Delicatessen的相关性，第四个维度体现了Fresh和Delicatessen的相关性，由此可见6个特征被主要分成了2组，其中Milk、Grocery、DP代表的是咖啡店，Fresh、Frozen、Delicatessen代表的是零售商 ### 观察 运行下面的代码，查看经过对数转换的样本数据在进行一个6个维度的主成分分析（PCA）之后会如何改变。观察样本数据的前四个维度的数值。考虑这和你初始对样本点的解释是否一致。
...
#   
# - 高斯混合模型聚类算法的优点
#   - 投影后样本点不是一个确定的分类标记，而是得到每个类的概率，这是一个重要信息
#   - 计算快，是学习混合模型最快的算法
#   - 低偏差，不容易欠拟合
# - 由于Kmeans对于outlier比较敏感，本数据集中outliers比较多，故我选择高斯混合模型聚类算法
# 
...
# **回答:**
# 主要是咖啡店，因为他们都属于Cluster 1，并且他们的中心点Segment 1的值牛奶和、杂物和清洁纸的量超过了75%而且超过了平均数，而其他的数量较少，咖啡店牛奶和纸巾的消耗必然很多，所以更有可能是咖啡店不相符
# 
